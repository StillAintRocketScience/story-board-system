...
> **3-line summary:** Adds §5.7 Shot Grammar Extension — the dimensions §5.2's intent taxonomy does not carry: an explicit **angle** dimension, the missing **size rungs**, and a wholly new **subject-state** axis (facing / expression / gesture). Scoped deliberately narrow: §5.7 **excludes every token §5.2 already owns**, so no token means two things. Additive, back-compatible — MINOR per §9.1. **Needs your decision:** sign-off to Approved; the parked 2026-07-04 `storyboard-prompt-builder` consolidation stays SEPARATE and unresolved (§5.7 provenance note).
>
> *v1.3 summary (retained):* Adds Mode D ("Website") — converts a business/IP brief or URL into a Website Prompt Manifest (hero-image prompt + per-clip motion prompts, Higgsfield-decoupled) plus a Site Build Brief for downstream execution. Serves two consumers via thin adapters: NAICS client sites and SARS IP Lookbook sites. Needs your decision: none — Approved 2026-07-04 after paper validation (moded_website_validation_run.md); next action is the first production Mode D run.
...
STATUS:             Approved (2026-09-23 — Sir sign-off, conditioned on
                    validation first. Validation FAILED on first pass
                    (10/15 real frames expressible); 4 repairs applied;
                    re-test 15/15. Evidence:
                    storyframe-v_v1.4_shotgrammar_validation_run.md.
                    v1.3 remains Approved and unchanged on disk.)
...
```
[v1.4.0 — 2026-09-23]
- Added §5.7 Shot Grammar Extension v1.0. It EXTENDS §5.2's existing
  canonical shot taxonomy (15 tokens, v1.0/v1.1, retained by reference in
  v1.2 and v1.3). It does NOT replace it and is NOT the family's first shot
  vocabulary - §5.2 already was.
- §5.7 adds only what §5.2 lacks: (a) an explicit ANGLE dimension -
  low/high/dutch/eye_level all at ZERO occurrences in the canon specs
  today; (b) the missing size RUNGS (medium, close_up, extreme_close_up,
  extreme_wide); (c) a SUBJECT-STATE axis (facing/expression/gesture),
  which the family has never carried; (d) two relational tokens for the
  multi-subject case Axis 2 cannot express.
- SCOPE TRIM (same day, Sir's call "A"): §5.7 deliberately EXCLUDES the
  five tokens §5.2 already owns - `wide`, `macro`, `establishing`,
  `tracking`, `orbit`. A first draft of §5.7 duplicated them, which would
  have made a token like `macro` mean an intent in §5.2 and a size in
  §5.7. Rule R0 now makes that exclusion binding. Axis 1 went from 33
  tokens to 19 in the same pass; speculative movement tokens with no
  identified need (whip_pan, zoom, rack_focus, crane, steadicam,
  locked_off, insert, pov, top_down) were cut.
- Camera tokens counted across the five canon specs ONLY (v1.1, v1.2, v1.3,
  syncframe-v_v0.1, songmap-v_v0.2). `storyboard-prompt-builder.md` was
  EXCLUDED from the count and was not a source - it carries a standing
  do-not-fork instruction.
- Subject-state axis harvested from `10-ACTIVE/PERSONA_SHEET_METHOD_v0.2`
  §3 via `10-ACTIVE/STORYBOARD_SHOT_VOCAB_GAP_v0.2`.
- §4.2: `frames[].subject_note` added (optional). `camera_note` and
  `camera_language[]` gain a controlled preferred vocabulary. All additive
  - v1.3 consumers emitting free prose are unaffected.
...
- Sir conditioned sign-off on paper validation first. The run FAILED on its
  ...
  storyframe-v_validation_run.md) left only 10/15 camera notes expressible.
  The scope trim had gone too far. Four repairs:
  R-1 restored `locked_off` and `top_down` - cut as "synonyms" but both
      used VERBATIM in produced work as deliberate choices.
  R-2 added rule R0.1 - the `wide` lens-vs-intent disambiguation, an R0
      edge case two real frames hit.
  R-3 added an Axis 2 `Head` dimension (head_up/head_down/head_tilt) -
      without it, 3 of the Persona Generator's 22 directives could not be
      expressed.
  R-4 documented `glide`/`lateral slide` as an accepted approximation to
      `dolly` rather than inventing a token below R6's evidence bar.
- Re-test after repairs: 15/15 accounted, 12 clean + 2 accepted
  approximations + 1 out of grammar scope (frame rate). Axis 2: 22/22.
  Collisions with §5.2: 0. §5.2 byte-identical to v1.3.
- STATUS: Draft -> Approved on that result.
...
OUTPUT_SCHEMA (v1.4 additions):
{
  ...
  "frames": [
    {
      ...                                    // v1.2 frame fields retained
      "subject_note":       string?          // NEW v1.4 - subject-state axis
                                             // (facing / expression / gesture)
                                             // per §5.7 Axis 2. Optional.
    }
  ]
}
```

**§5.7 binding (new v1.4).** `frames[].camera_note` SHOULD draw its tokens from §5.7 **Axis 1**
(camera); the new `frames[].subject_note` SHOULD draw from **Axis 2** (subject state); `camera_language[]`
SHOULD draw from Axis 1. The §5.2 intent taxonomy continues to tag the frame's PURPOSE and is unaffected —
a frame may legitimately carry a §5.2 intent AND §5.7 grammar at once. "SHOULD", not "MUST" — free prose
remains valid so v1.3 consumers keep v1.3 behavior (§9.1 MINOR test). Never mix axes inside one field
(§5.7 R3), and never re-scope a §5.2 token into §5.7 (R0).
...
### 5.7 Shot Grammar Extension v1.0 *(new v1.4)*

#### Relationship to §5.2 — read this first

The family **already has** a canonical shot taxonomy. §5.2 carries it (*"Shot taxonomy (canonical set
v1.0, plus v1.1 additions)"*), established in v1.1, and v1.3 retains it by explicit reference:

`establishing` · `reveal` · `macro` · `hero` · `detail` · `tracking` · `wide` · `impact` · `orbit` ·
`light_sweep` · `environmental` · `endcard` · `reaction` · `comparison` · `testimony`

**§5.7 does not replace §5.2 and is not the family's first shot vocabulary.** §5.2 remains the shot-
**intent** vocabulary — what a frame is *for*. §5.7 supplies only the dimensions §5.2 has no way to
express, and **deliberately holds no token that §5.2 already owns** (R0).

#### What §5.2 is, and the three gaps this closes

§5.2's 15 tokens mix three kinds of thing: **framing** (`wide`, `macro`, `detail`, `establishing`),
**movement** (`tracking`, `orbit`, `light_sweep`) and **narrative intent** (`reveal`, `impact`, `reaction`,
`testimony`, `comparison`, `endcard`). That works as an intent vocabulary, but a frame can be tagged `hero`
with its size, its angle, and whether the subject faces camera all unstated.

| # | Gap | Evidence across the five canon specs |
|---|---|---|
| **1** | **No angle dimension** | `low angle` / `high angle` / `dutch` / `eye level` / `top-down` — **all 0 occurrences**. The word `angle` appears 5×, only ever as *"hero-angle"*. Corroborated independently: `storyboard-prompt-builder.md` reached for *Low Angle / High Angle* when someone actually sat down to write storyboard prompts. |
| **2** | **No size rungs** | `medium` = **0**, `close_up` = **1**. §5.2's `wide` / `macro` / `detail` are intents with nothing between them. |
| **3** | **No subject-state axis** | Nothing in the family describes facing, expression or gesture. The Persona Generator exists partly because this vocabulary had nowhere to live. |
...
> established practice — most of the angle dimension is `[ADDED]`, which is precisely gap 1.

#### Axis 1 — CAMERA (what the lens does) — 21 tokens, §5.2's own excluded

| Dimension | Tokens |
|---|---|
| **Size** *(rungs only)* | `close_up` [1] · `extreme_close_up` [1] · `medium` **[ADDED]** · `extreme_wide` **[ADDED]** |
| **Angle** | `overhead` [2] · `top_down` [1†] · `aerial` [1] · `low_angle` [1†] · `high_angle` **[ADDED]** · `dutch` **[ADDED]** · `eye_level` **[ADDED — unmarked default]** |
| **Movement** | `handheld` [3] · `static` [2] · `dolly` [2] · `push_in` [3†] · `locked_off` [1†] · `pan` [1†] · `pull_out` **[ADDED]** · `tilt` **[ADDED]** |
| **Relational** | `over_the_shoulder` **[ADDED]** · `two_shot` **[ADDED]** |

**Excluded on purpose (R0):** `wide`, `macro`, `establishing`, `tracking`, `orbit` — §5.2 owns these.
**Cut as speculative:** `whip_pan`, `zoom`, `rack_focus`, `crane`, `steadicam`, `insert`, `pov`. Any can
return through R6 once a real build reaches for it twice.

**† Restored by validation.** `locked_off` and `top_down` were cut as "synonyms" of `static` and
`overhead` in the first trim. The v1.4 validation run retro-tagged a real produced 15-frame set and
found **both used verbatim** — *"Locked-off product lens"*, *"Tight top-down macro"* — as deliberate
creative choices, not defaults. They are back. Counts marked `†` come from that frame set
(`storyframe-v_validation_run.md`), not from the five spec files. Evidence:
`storyframe-v_v1.4_shotgrammar_validation_run.md`.

**Accepted approximation (documented, not repaired):** *"Macro lens glide"* and *"Slow lateral slide"*
map to `dolly` plus prose. A dedicated lateral token (`truck`) appeared once, below R6's ≥2× bar, so it
was not invented. R6 is its route in.

#### Axis 2 — SUBJECT STATE (what the subject does) — *entirely new; no prior family vocabulary*

> **Validation note.** Axis 2 could not be retro-tagged against family output: **no character frame set
> exists anywhere in the family** — every validation run to date is product (Nike AF1) or architectural
> (The Strategist). It was instead validated by derivation against the Persona Generator's 22 working
> shot directives, which exposed a missing **Head** dimension — 3 of those 22 (*looking up*, *looking
> down*, *head tilted*) were unexpressible without it. Now 22/22 express.

| Dimension | Tokens |
|---|---|
| **Facing** | `front` · `three_quarter_left` · `three_quarter_right` · `profile_left` · `profile_right` · `back` |
| **Head** *(added by validation)* | `head_up` · `head_down` · `head_tilt` |
| **Expression** | `neutral` · `soft_smile` · `laugh` · `surprise` · `thinking` · `excited` · `sleepy` · `singing` |
| **Gesture** | `standing_front` · `standing_back` · `walking` · `sitting` · `waving` · `pointing` · `jumping` · `dancing` |

#### Why axes and not one longer list

§5.2 already shows the cost of a flat list: framing, movement and intent sit as siblings, so the
vocabulary cannot specify a frame unambiguously. §5.7 keeps its dimensions separate. A frame is
`[§5.2 intent] + [size · angle · movement] × [facing · expression · gesture]`. **Unspecified subject-facing is
where inter-panel continuity and identity drift originate** — the same drift `GENERATION_LADDER_PROTOCOL`
§3.1 and `COMFY_YIELD_SYSTEM` fight at the CREF layer.

#### Rules

- **R0 — §5.2 owns its tokens; §5.7 never redefines one.** `wide`, `macro`, `establishing`, `tracking`
  and `orbit` keep their §5.2 intent meaning and are absent from Axis 1 by design. No token may mean one
  thing in §5.2 and another in §5.7. A frame needing both tags one intent from §5.2 and its grammar from
  §5.7.
- **R0.1 — the `wide` disambiguation** *(added by validation)*. R0 governs the token **string**, and
  real frames use "wide" in two senses: §5.2's `wide` **intent** (*"a wide beat"*) and a **framing**
  sense (*"Low wide-angle shot"*, *"Wide cinematic angle"* — both in the validated frame set). Tag the
  intent sense `wide` in §5.2. For the framing sense use the Axis 1 size rungs (`extreme_wide`, or
  `medium`/`close_up` as the frame warrants) plus the angle token — **never re-use the string `wide`
  inside Axis 1.**
- **R1 — Preferred, not enforced.** Draw from these tokens where one fits; free prose stays valid. A hard
  validator would break v1.3 behavior and forfeit the §9.1 MINOR classification.
- **R2 — At most one token per dimension per frame.** Two sizes in one frame is a spec error.
- **R3 — Never mix axes in one field.** Axis 1 → `camera_note` / `camera_language[]`. Axis 2 →
  `subject_note`. §5.2 intent → wherever it is recorded today. (§4.2 binding.)
- **R4 — `eye_level` and `static` are unmarked defaults.** Omit rather than state them.
- **R5 — Axis 2 is single-subject by construction.** A multi-subject frame uses `over_the_shoulder` or
  `two_shot` plus one `subject_note` per named subject, or prose.
- **R6 — A new token needs evidence.** A term appearing ≥2× in real builds without a fit here triggers a
  MINOR taxonomy addition (§9.2), not an ad-hoc inline invention. This is the route back for anything cut
  above.
- **R7 — §5.2 is untouched.** Retagging or retiring any §5.2 token is out of scope for v1.4 and would be
...
**v1.4 qualifies as MINOR on the same test:** §5.7 adds no blocking step to any existing flow, `frames[].subject_note` is additive and optional, the vocabulary is preferred rather than enforced (§5.7 R1), §5.2 is untouched and its tokens are excluded from §5.7 (R0/R7), and a v1.3 consumer emitting free-prose `camera_note` gets v1.3 behavior unchanged.
